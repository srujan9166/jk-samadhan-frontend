...
export default function GrievanceModal({ isOpen, onClose, mode = 'grievance', onLoginRedirect, onRegisterRedirect, isLoggedIn, user, onGrievanceSubmit }) {
      ...
      try {
        ...
        if (divRes.ok) {
          ...
          for (const div of divisions) {
            const distRes = await fetch(`${API_BASE_URL}/api/geo/divisions/${div.id}/districts`);
            if (distRes.ok) {
  ...
  const districts = districtsList.map(d => d.name);
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto motion-modal-overlay ${isOpen
      ...
      <div className={`bg-white rounded-2xl shadow-2xl max-w-2xl w-full overflow-hidden motion-modal-content transform flex flex-col max-h-[90vh] ${isOpen ? 'scale-100 translate-y-0 opacity-100' : 'scale-95 translate-y-4 opacity-0'
        }`}>
        {isGated ? (
